...
from .sladd import sladd_get_convexhull
...
class SBI_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8, raw=True,sladd=False):
	...
	def self_blending(self,img,landmark):
		...
		if self.sladd:
			...
			mask = sladd_get_convexhull(img, landmark, region_idx)
		else:
			if exist_bi:
