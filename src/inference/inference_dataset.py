...
def main(args):
    ...
    elif args.dataset == 'FFM':
        video_list, target_list = init_ff_mine(raw=False, data_types=args.fpp_types)
    elif args.dataset == 'FFR':
        video_list, target_list = init_ff_mine(raw=True, data_types=args.fpp_types)
    ...
    if args.run_test:
        video_list = video_list[:1] + video_list[-1:]
        target_list = target_list[:1]+target_list[-1:]
    output_list = []
    for filename in tqdm(video_list):
        try:
            # extract face frame from video(may have multiple faces in a single frame.)
            face_list, idx_list = extract_frames(
                filename, args.n_frames, face_detector
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', dest='fpp_types', default="DF,F2F,FS,NT,real", type=str)
    args = parser.parse_args()
    args.fpp_types = args.fpp_types.split(",")
    print(args)
    main(args)
